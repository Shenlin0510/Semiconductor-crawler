...
class Email():
    ...
    def __init__(self, sender, recipient, producible):
        
        self.producible = producible
        self.msg = MIMEMultipart()
        self.msg['Subject'] = "Automatic Semiconductor News."
        self.msg['From'] = sender
        self.msg['To'] = ", ".join(recipient)
        self.msg.add_header('Content-Type', 'text/html; charset=utf-8')
    ...
    def get_data(self):
        ...
        '''
        html_title ='''<h3><b>The Analyzed Htmls of Automatic Semiconductor News.</b></h3>'''
        ...
        #database
        conn = lite.connect(fr"{os.getcwd()}\databases\2024-04-26_crawled_sorted.db")
        cur = conn.cursor()
        ...
        for table in tables:
            ...
            if table[0].strip().lower() in Booming_list:
                ...
                boom_titles = booming["title"].astype(str).values
               
                df_boom = pd.concat([df, booming], ignore_index=True)
            ...
            # Mature Application
                ...
                mature_titles = mature["title"].astype(str).values

                df_mature = pd.concat([df, mature], ignore_index=True)
            ...
            #Foundry
                ...
                df_foundry = pd.concat([df, foundry], ignore_index=True)
        ...
        boom_title = [title for title in boom_titles]
        mature_title = [title for title in mature_titles]
        foundry_title = [title for title in foundry_titles]


        #Booming Application
        boom_first_new, boom_new = [],[]
        titles_boom = df_boom["title"].astype(str).values
        urls_boom = df_boom["url"].astype(str).values
        sums_boom = df_boom["summary"].astype(str).values
        ...
        titles_mature = df_mature["title"].astype(str).values
        urls_mature = df_mature["url"].astype(str).values
        sums_mature = df_mature["summary"].astype(str).values
        
        filter_mature.extend(boom_title)
        ...
        titles_foundry = df_foundry["title"].astype(str).values
        urls_foundry = df_foundry["url"].astype(str).values
        sums_foundry = df_foundry["summary"].astype(str).values
        
        filter_foundry.extend(boom_title)
        filter_foundry.extend(mature_title)
        ...
        topical_titles.extend(boom_title)
        topical_titles.extend(mature_title)
        topical_titles.extend(foundry_title)
    ...
    def sending(self):
        s = smtplib.SMTP("10.1.254.112") #郵件主機的server ip
...
if __name__ == '__main__':
    ...
    with open(fr"{os.getcwd()}\adjustments\sender.txt", "a+") as sed:
        sed.seek(0) #從頭開始讀取
        sender = sed.readline()


    recipient_list = []
    with open(fr"{os.getcwd()}\adjustments\recipient.txt", "a+") as rec:
        rec.seek(0) #從頭開始讀取
    ...
    email = Email(sender, recipient_list, True)
    email.get_data()
    email.sending()
